            ...
            if data:
                ...
                for item in data:
                    ...
                    if "Error" in dict_item.keys():
                        ...
                        #request_id_query = Constants.QUERIES['CARA_REPORT_REQUEST_ID'].format(dict_item["address"])
                        #print("Query:", request_id_query)
                        #request_id = self.__trdb_api.get_query(request_id_query)
                        ...
                        blockchain = dict_item["blockchain"]
                        if str(dict_item["blockchain"]) == "bsc":
                            total_amt_bc = total_amt["bnb"]
                        else:
                            total_amt_bc = total_amt[blockchain]
                        estimated_mal_amt = dict_item["estimated_mal_amt"]
                        if str(blockchain) == "bsc":
                            mal_amt_bc = estimated_mal_amt["bnb"]
                        else:
                            mal_amt_bc = estimated_mal_amt[blockchain]
                        ...
                        data_dict = (dict_item["address"], dict_item["risk_score"], prv_risk_score, dict_item["analysis_start_time"],
                                     ...
                                     links_string, funds_string, blockchain)
                    ...
                    print("Query: ", cara_report_insert_query)
            ...
            if number_new_indicators != 0:
                for indicator in new_indicators:
                    ...
                    if str(indicator['pattern_subtype']) == 'ETH':
                        ...
                        blockchain = 'xrp'
                    elif str(indicator['pattern_subtype']) == 'BSC':
                        blockchain = 'bsc'
                    else:
                        blockchain = 'bch'
                    updated = datetime.datetime.fromtimestamp(int(indicator['updated']))
