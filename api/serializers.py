            ...
            with transaction.atomic():
                ...
                for indi_item in indicators_data:
                    if "uid" in indi_item:
                        indicator = models.Indicator.objects.get(
                            uid=indi_item["uid"])
                        ind_queryset = models.Indicator.objects.filter(uid=indi_item["uid"])
                        ...
                        if indicator.security_category != indi_item["security_category"].value:
                            ind_queryset.update(security_category=indi_item["security_category"].value)
                        if indicator.pattern_type != indi_item["pattern_type"].value:
                            ind_queryset.update(pattern_type=indi_item["pattern_type"].value)
                        if indicator.pattern_subtype != indi_item["pattern_subtype"].value:
                            ind_queryset.update(pattern_subtype=indi_item["pattern_subtype"].value)
                        if "s_tags" in indi_item and indicator.s_tags != indi_item["s_tags"]:
                            ind_queryset.update(s_tags=indi_item["s_tags"])
                        if "environment" in indi_item and indicator.environment != indi_item["environment"]:
                            ind_queryset.update(environment=indi_item["environment"])
                        if "detail" in indi_item and indicator.detail != indi_item["detail"]:
                            ind_queryset.update(detail=indi_item["detail"])
                        if "annotation" in indi_item:
                            new_annotations = [x.strip()
                                               for x in indi_item["annotation"].split(",")]
                            prev_annotations = [
                                annotation.annotation for annotation in indicator.annotations.all()]
                            if new_annotations != prev_annotations:
                                ...
                                for annotation in new_annotations:
                                    ...
                                    models.IndicatorAnnotation.objects.create(
                                        indicator=indicator, annotation=anno)
                            ind_queryset.update(annotation=indi_item["annotation"])
