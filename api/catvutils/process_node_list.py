...
class ProcessNodeList:
    ...
    def create_node_list_by_depth(self):
        ...
        if self.src_depth > 0:
            self._async_source_nodes_by_level = pool.apply_async(self.process_node_list, ('src',)).get()
        ...
        pool.close()
        pool.join()
        # print(self._async_source_nodes_by_level)
        # print(self._async_dist_nodes_by_level)
    ...
    def process_node_list(self, mode):
        ...
        if mode == 'src':
            ...
            while src_level <= self.src_depth:
                temp_src_node_list = [src_node for src_node in self.src_nodes if src_node['level'] == src_level]
