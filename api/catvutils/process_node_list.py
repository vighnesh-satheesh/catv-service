...
class ProcessNodeList:
    def __init__(self, node_list, depths):
        ...
        self._async_source_nodes_by_level = []
        self._async_dist_nodes_by_level = []
        self.source_nodes = [node for node in self.node_list if node['level']<=0]
    ...
    def create_node_list_by_depth(self):
        ...
        if self.src_depth>0:
            self._async_source_nodes_by_level = pool.apply_async(self.process_node_list, ('src',))
        if self.dist_depth>0:
            self._async_dist_nodes_by_level = pool.apply_async(self.process_node_list, ('dist',))
        pool.close()
        pool.join()
        print(self._async_dist_nodes_by_level.get())
    ...
    def process_node_list(self, mode):
        if mode == 'dist':
            ...
            while dist_level<=self.dist_depth:
                temp_dist_node_list = [dist_node for dist_node in self.dist_nodes 
                                                    if dist_node['level']==dist_level]
                dist_nodes_by_level.append(temp_dist_node_list)
                dist_level = dist_level + 1
            print(dist_nodes_by_level)
        ...
        if mode == 'src':
            ...
            while src_level<=self.src_depth:
                temp_src_node_list = [src_node for src_node in self.src_nodes 
                                                    if src_node['level']==src_level]
                src_nodes_by_level.append(temp_src_node_list)
                src_level = src_level + 1  
            return src_nodes_by_level  

    def get_src_node_lists(self):
        return self._async_source_nodes_by_level     
