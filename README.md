...
4. Set two enviroment variables. **PORTAL_API** and **PORTAL_API_PATH**.
   ...
   ```
   $ export PORTAL_API=development
   $ export PORTAL_API_PATH=/env/file/path.env
   ```

5. Create _.env_ file at the path stored at **PORTAL_API_PATH**. Please check _sample.env.example_ and refer to Environment Variables.
...
1. The `docker-compose.yml` file defines a webapi service which does the following:
   ...
   3. Define a few environment variables, which are used during the project initialization phase by Django.
      Most importantly, `PORTAL_API_PATH` & `API_TOKEN_ENCRYPT_PRIVATE_KEY`. You need to grab a copy of them from someone
      if you don't have it. If you already have the files, then inside the file for the `PORTAL_API_PATH` change the DATABASE
